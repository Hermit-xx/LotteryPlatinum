...
system("title 白金之星-双色球另类分析工具-v1.1")
...
def update(app_key):
    ...
    if not os.path.isdir('data\\ssq\\part'):
        print('数据目录不存在，自动创建')
        os.makedirs('data\\ssq\\part')

    f = open('data\\ssq\\part\\1.txt', 'w')
    ...
    for index in range(2, total_page):
        ...
        f = open('data\\ssq\\part\\' + str(index) + '.txt', 'w')
...
def find():
    while True:
        no = input('====================\n'
                   '输入号码，先输入红球，红球大小无需排序，最后输入蓝球，每个数字之间用空格分隔。\n'
